...
启动页新增一行「XXMI 安装位置」+「更改…」+「用默认」，选中的路径存进
`ModManagerOptions.XxmiRootFolderPath`，供之后的每次一键配置沿用 —— 否则重复跑会往默认位置再装一份。

1. 默认态：首启页显示「默认（游戏所在盘符下的 XXMI 文件夹）」，「用默认」按钮**不可见**（尚无自定义路径）
2. 点「更改…」选一个**非游戏所在盘**的目录（如游戏在 C:、选 `D:\XXMI-Test`）→ 文本变成该路径，
   「用默认」按钮出现；状态栏按新位置重新检测（新目录为空 → 仍提示未安装，属预期）
3. 点「配置 / 更新 / 修复 Mod 环境」→ 向导里 `RootFolder` 应是 `D:\XXMI-Test`（而非 `C:\XXMI`），
   安装完成后首启页两个路径选择框自动填入的是新位置下的 `3Dmigoto` / `Mods`
4. 点 Save 后重启 app → 首启页仍显示 `D:\XXMI-Test`（已持久化），再跑一键配置不产生第二份安装
5. 「用默认」→ 文本回到默认描述、按钮消失；此时**不点 Save 直接重启**仍回到自定义路径（未保存不生效）
6. 设置页的一键配置入口复用同一路径：先在首启页选自定义目录并 Save，再去设置页跑向导 →
   同样是 `D:\XXMI-Test`（设置页没有自己的选择器，读的就是这个字段）
